...
def involved(courses, person):
    dictio = {}
    for when in courses: # feb2012
        info = []
        for course in courses[when]: # cs101
            for data in courses[when][course]: #name,
                if courses[when][course][data] == person:
                       info.append(course);
            if len(info) > 0:
                dictio[when] = info                       
